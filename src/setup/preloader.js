...
class PreloaderExperience {
    ...
    this.boundOnContextRestored = this.onContextRestored.bind(this);
        ...
        this.parallaxFactor = 0.02; // ridotto per effetto meno accentuato
    ...
    async init() {
        ...
        this.camera.position.z = window.innerWidth < 768 ? 8 : 15;
        this.camera.fov = window.innerWidth < 768 ? 85 : 75;
        this.camera.updateProjectionMatrix();
        // Limit pixel ratio on mobile to prevent canvas from being too large and cut off
        const maxPixelRatio = window.innerWidth < 768 ? 2 : window.devicePixelRatio;
        ...
        try {
            const mod = await import('../core/rendering/webglUtils.js');
    ...
    createMeshes() {
        ...
        } else if (this.cursorName === 'Glitch') {
            tutorialTitleText = 'SYSTEM STATUS: [MATRIX RAIN PROTOCOL]';
            tutorialLines = [ "> EFFETTO: ATTIVO", ">   MUOVI MOUSE -> NAVIGA NELLA MATRICE", ">   TRASCINAMENTO -> TRACCIA DIGITALE", "> ", "> PROTOCOLLO:", ">   OSSERVA LA PIOGGIA DI CODICE.", ">   IL CURSORE È LA TUA GUIDA NEL SISTEMA.", "> ", "> ... BENVENUTO NELLA MATRICE." ];
    ...
    createMatrixParticles() {
        ...
        const columns = 60; // aumentato a 60 colonne per copertura migliore
        const particlesPerColumn = 30; // aumentato a 30 particelle per colonna (60 * 30 = 1800 particelle)
        const screenHeight = 80; // altezza totale aumentata
        const columnSpacing = 2.0; // spaziatura ridotta a 2.0 per densità maggiore
        const startX = -(columns - 1) * columnSpacing / 2; // posizione x iniziale centrata

        for (let col = 0; col < columns; col++) {
            for (let row = 0; row < particlesPerColumn; row++) {
                const char = matrixChars[Math.floor(Math.random() * matrixChars.length)];
                const geo = new TextGeometry(char, { font: this.fontAlien, size: 0.6, height: 0.02 }); // ridotto da 0.7 a 0.6 per densità maggiore
                const mat = new THREE.MeshStandardMaterial({ color: 0x00ff00, emissive: 0x004400, emissiveIntensity: 0.5, transparent: true, opacity: 1.0 });
                const mesh = new THREE.Mesh(geo, mat);

                // Posizione regolare: colonna + offset casuale minimo per naturalezza
                const x = startX + col * columnSpacing + (Math.random() - 0.5) * 0.3; // offset ridotto
                const y = (row / (particlesPerColumn - 1) - 0.5) * screenHeight + (Math.random() - 0.5) * 1; // jitter ridotto
                const z = -15 + Math.random() * 3; // profondità ridotta

                mesh.position.set(x, y, z);

                mesh.userData.speed = 0.07; // velocità completamente uniforme per flusso perfetto
                mesh.userData.glitchTimer = Math.random() * 1000;
                mesh.userData.originalChar = char;
                mesh.userData.opacity = 0.6;
                mesh.userData.column = col; // salva colonna per riferimento
                this.matrixGroup.add(mesh);
                this.matrixParticles.push(mesh);
            }
        }
    }
    ...
    addEventListeners() { 
        EventBus.on('global:mousemove', this.boundOnMouseMove); 
        EventBus.on('global:resize', this.boundOnWindowResize); 
        this.preloaderElement.addEventListener('click', this.boundOnPreloaderClick); 
        EventBus.on('global:keydown', this.boundHandleSystemCommands);
    ...
    onWindowResize() { 
        ...
        this.camera.fov = this.sceneContainer.clientWidth < 768 ? 85 : 75;
        this.camera.position.z = this.sceneContainer.clientWidth < 768 ? 8 : 15;
        this.camera.updateProjectionMatrix(); 
        this.renderer.setSize(this.sceneContainer.clientWidth, this.sceneContainer.clientHeight); 
        this.composer.setSize(this.sceneContainer.clientWidth, this.sceneContainer.clientHeight);
        // Scale the scene to fit smaller screens
        const baseWidth = 1920;
        const baseHeight = 1080;
        const scaleFactor = Math.min(this.sceneContainer.clientWidth / baseWidth, this.sceneContainer.clientHeight / baseHeight, 1);
        this.sceneGroup.scale.setScalar(scaleFactor);
        this.tutorialGroup.scale.setScalar(scaleFactor);
    }
    
    onDeviceOrientation(event) {
        if (!event.beta || !event.gamma) return;
        // Use gyroscope to subtly rotate the scene group for 3D movement
        const betaRad = (event.beta - 90) * Math.PI / 180 * 0.05; // Pitch
        const gammaRad = event.gamma * Math.PI / 180 * 0.05; // Roll
        this.sceneGroup.rotation.x = betaRad;
        this.sceneGroup.rotation.y = gammaRad;
    }
    ...
    animate() {
        ...
        // Always apply parallax rotation to the active group (scene or tutorial)
        ...
        if (this.matrixActive) {
            this.matrixParticles.forEach(particle => {
                ...
                particle.userData.glitchTimer -= 16; // assuming 60fps
                if (particle.userData.glitchTimer < 0) {
                    particle.userData.glitchTimer = 1000 + Math.random() * 2000;
                    // Glitch effect: change text
                    const glitchChars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
                    const newChar = Math.random() > 0.7 ? glitchChars[Math.floor(Math.random() * glitchChars.length)] : particle.userData.originalChar;
                    if (particle.geometry) particle.geometry.dispose();
                    particle.geometry = new TextGeometry(newChar, { font: this.fontAlien, size: 0.6, height: 0.02 });
                    particle.geometry.center();
                }
                // Pulsing scale
                const pulse = 1 + Math.sin(Date.now() * 0.005 + particle.position.x) * 0.2;
                particle.scale.setScalar(pulse);
                // Color glitch
                if (Math.random() > 0.95) {
                    particle.material.color.setHex(Math.random() * 0xffffff);
                    particle.material.emissive.setHex(Math.random() * 0x444444);
                } else {
                    particle.material.color.setHex(0x00ff00);
                    particle.material.emissive.setHex(0x004400);
                }
                if (particle.position.y < -45) { // quando esce completamente dallo schermo (adattato alla nuova altezza)
                    particle.position.y += 90; // ricompare dall'alto (45 + 45 = 90)
                    // mantiene posizione x (colonna) e z invariate per continuità perfetta
                    particle.rotation.z = Math.random() * Math.PI * 2; // solo rotazione casuale
                    particle.scale.setScalar(1);
                    // Reset to original char
                    if (particle.geometry) particle.geometry.dispose();
                    particle.geometry = new TextGeometry(particle.userData.originalChar, { font: this.fontAlien, size: 0.6, height: 0.02 });
                    particle.geometry.center();
                }
            });
            // Apply same parallax rotation as the main scene
            const rotationIntensity = 0.08;
            const targetRotationX = -this.mouseVec.y * rotationIntensity;
            const targetRotationY = -this.mouseVec.x * rotationIntensity;
            this.matrixGroup.rotation.x += (targetRotationX - this.matrixGroup.rotation.x) * 0.05;
            this.matrixGroup.rotation.y += (targetRotationY - this.matrixGroup.rotation.y) * 0.05;
        }
    ...
    destroy() {
        ...
        EventBus.off('global:mousemove', this.boundOnMouseMove);
        EventBus.off('global:resize', this.boundOnWindowResize);
        this.preloaderElement.removeEventListener('click', this.boundOnPreloaderClick);
        EventBus.off('global:keydown', this.boundHandleSystemCommands);
        if (window.DeviceOrientationEvent) {
