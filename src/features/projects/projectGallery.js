        ...
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (slider && !slider.dataset.videosLoaded) {
                    ...
                    const beforeVideo = slider.dataset.videoBeforeMp4;
                    const afterVideo = slider.dataset.videoAfterMp4;
                    
                    if (beforeVideo && beforeVideo.trim()) {
                        slider.querySelector('.video-background').appendChild(createVideo(beforeVideo));
                    }
                    if (afterVideo && afterVideo.trim()) {
                        slider.querySelector('.video-foreground').appendChild(createVideo(afterVideo));
                    }
            ...
            <a href="#" data-project-id="${p.id}" class="project-card reveal" data-tools="${p.tools.join(', ')}">
                <div class="card-inner">
                    ...
                    <div class="card-visual">
                        ${p.poster ? `<img src="${p.poster}" alt="Poster: ${p.title}" class="poster-image" loading="lazy">` : ''}
